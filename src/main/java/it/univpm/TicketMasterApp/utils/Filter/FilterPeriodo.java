...
public class FilterPeriodo {
	...
	public JSONObject trimestrale() {
		...
		for(Eventi e: evento) {
			...
			if(data_evento.isBefore(anno_prossimo) && data_evento.isAfter(oggi)) {// se la data che analizzo è prima di un
				//anno a partire da oggi e viene dopo la dato di oggi
